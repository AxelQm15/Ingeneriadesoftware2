...
function togglePlayPause(){
  if (currentSongIndex === null){ playSong(0); return; }
...
function toggleShuffle(){
  ...
  if (isShuffleActive){
    ...
    songs = [...originalSongs];
    currentSongIndex = songs.findIndex(s => s.audioSrc === currentSongObject?.audioSrc);
  }
  renderPlaylist();
}
...
function shuffleSongs(){
  const currentSongObject = songs[currentSongIndex];
  let remainingSongs = originalSongs.filter(s => s.audioSrc !== currentSongObject?.audioSrc);
...
function renderPlaylist(){
  ...
  songs.forEach((song, index) => {
    ...
    // Cols:
    //  - XS: [# + play] | [cover + título + artista debajo] | [...] (sin encabezado)
    //  - MD+: tabla clásica con columnas
    playlistSongsContainer.innerHTML += `
      <div class="row song-row g-0 ${index === currentSongIndex ? 'playing' : ''}" id="song-${index}" onmouseleave="hideMenuWithDelay(${index})">
        <!-- Índice / Play -->
        <div class="col-2 col-md-1 text-center d-flex align-items-center justify-content-center position-relative">
          <span class="song-index">${index + 1}</span>
          <button class="btn-play" onclick="playSong(${index})" aria-label="${isThisSongPlaying ? 'Pausar' : 'Reproducir'}">
            ${isThisSongPlaying ? '❚❚' : '▶'}
          </button>
        </div>

        <!-- Título (XS: incluye artista debajo) -->
        <div class="col-8 col-md-4 d-flex align-items-center">
          ...
          <div class="song-title-artist">
            ...
            <span class="song-artist-mobile d-md-none" title="${song.artist}">${song.artist}</span>
          </div>
        </div>

        <!-- Artista (solo MD+) -->
        <div class="d-none d-md-flex col-md-3 align-items-center song-artist" title="${song.artist}">${song.artist}</div>

        <!-- Álbum (solo LG+) -->
        <div class="d-none d-lg-flex col-lg-3 align-items-center song-album" title="${song.album}">${song.album}</div>

        <!-- Menú -->
        <div class="col-2 col-md-1 text-center d-flex align-items-center justify-content-center">
          <div class="action-container">
            <button class="btn-context" onclick="toggleContextMenu(event, ${index})" aria-label="Abrir menú">…</button>
...
function removeSong(songIndex){
  ...
  if (confirm(`¿Seguro que quieres eliminar "${songs[songIndex].title}" de tu playlist?`)){
    ...
    if (originalIndex > -1) originalSongs.splice(originalIndex, 1);
    ...
    if (songIndex < currentSongIndex) currentSongIndex--;
    updateUI();
  }
}
...
function cancelHideMenu(){ clearTimeout(menuCloseTimer); }

function openNowPlayingModal(){ if (currentSongIndex !== null) nowPlayingModal.classList.add('active'); }
function closeNowPlayingModal(){ nowPlayingModal.classList.remove('active'); }
